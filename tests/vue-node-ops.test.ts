...
import { HOST_TYPES } from '../src/react/glyphis-node';

describe('Vue nodeOps', () => {
  it('createElement creates GlyphisNode with correct type', () => {
    const node = nodeOps.createElement('glyphis-view');
  ...
  it('createComment creates comment node', () => {
    ...
    expect(node.type).toBe('glyphis-comment');
  });

  it('insert appends child when no anchor', () => {
    const parent = nodeOps.createElement('glyphis-view');
    const child = nodeOps.createElement('glyphis-text');
  ...
  it('insert with anchor inserts before anchor', () => {
    const parent = nodeOps.createElement('glyphis-view');
    const first = nodeOps.createElement('glyphis-text');
    const second = nodeOps.createElement('glyphis-text');
    const inserted = nodeOps.createElement('glyphis-view');
  ...
  it('remove does nothing when node has no parent', () => {
    const orphan = nodeOps.createElement('glyphis-view');
    // Should not throw
  ...
  it('setElementText clears children and adds text child', () => {
    const node = nodeOps.createElement('glyphis-text');
    const existing = nodeOps.createElement('glyphis-view');
