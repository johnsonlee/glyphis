...
class GlyphisRuntime {
    ...
    private func setupBridge() {
        ...
        let setTimeout: @convention(block) (JSValue, Double) -> Int = { [weak self] callback, delay in
            ...
            self.nextRafId += 1
            self.rafCallbacks[id] = callback
            NSLog("[Glyphis] setTimeout registered id=%d delay=%.0fms callbacks=%d", id, delay, self.rafCallbacks.count)
            DispatchQueue.main.asyncAfter(deadline: .now() + delay / 1000.0) { [weak self] in
                guard let self = self else { return }
                NSLog("[Glyphis] setTimeout firing id=%d", id)
                if let cb = self.rafCallbacks.removeValue(forKey: id) {
    ...
    private func evaluate(_ js: String) {
        NSLog("[Glyphis] Evaluating JS (%d chars)", js.count)
        try? "EVAL_START:\(js.prefix(100))".write(toFile: "/tmp/glyphis-eval-debug.txt", atomically: true, encoding: .utf8)
        context.evaluateScript(js)
        NSLog("[Glyphis] JS evaluated")
    }
    ...
    func loadBundle() {
        ...
        // Fallback paths for development (run from repo root)
        let fallbackPaths = [
            "native/macos/GlyphisShell/Resources/bundle.js",
            ".glyphis-build/macos/bundle.js",
        ]
        ...
        let execRelativePath = (execDir as NSString).appendingPathComponent("bundle.js")
        if let js = try? String(contentsOfFile: execRelativePath) {
            evaluate(js)
            return
        }
