...
class GlyphRenderView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for cmd in renderCommands {
            ...
            case "opacity":
                ctx.saveGState()
                if let opacity = asFloat(cmd["opacity"]) {
    ...
    private func drawText(ctx: CGContext, cmd: [String: Any]) {
        ...
        let drawRect = CGRect(x: x, y: y + yOffset, width: w, height: lineHeight * 10)
    ...
    private func drawBorder(ctx: CGContext, cmd: [String: Any]) {
        guard let x = asFloat(cmd["x"]), let y = asFloat(cmd["y"]),
              ...
              let colors = cmd["colors"] as? [String], colors.count == 4 else { return }

        // Top
        if widths[0] > 0 {
            ctx.setStrokeColor(parseColor(colors[0]))
        ...
        // Right
        if widths[1] > 0 {
            ctx.setStrokeColor(parseColor(colors[1]))
        ...
        // Bottom
        if widths[2] > 0 {
            ctx.setStrokeColor(parseColor(colors[2]))
        ...
        // Left
        if widths[3] > 0 {
            ctx.setStrokeColor(parseColor(colors[3]))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        onTouch?("pressIn", p.x, p.y)
    }

    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let touch = touches.first else { return }
        let p = touch.location(in: self)
        onTouch?("pressOut", p.x, p.y)
        onTouch?("press", p.x, p.y)
    }
